from flask import render_template, session, redirect, url_for, abort,current_app,flash,request,make_response
from .. import db
...
def post(id):
    ...
    if page == -1:
        page = (post.comments.count()-1) / \
                current_app.config['ARTISAN_COMMENT_PER_PAGE']+1
    pagination = post.comments.order_by(Comment.timestamp.asc()).paginate(
        page,per_page=current_app.config['ARTISAN_COMMENT_PER_PAGE'],
        error_out=False)
